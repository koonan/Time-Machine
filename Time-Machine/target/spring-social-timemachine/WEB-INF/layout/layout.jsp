        ...
        <div class="collapse navbar-collapse navbar-ex1-collapse">
            ...
            <ul class="nav navbar-nav navbar-right">
                <sec:authorize access="isAnonymous()">
                    <li class ="signin_li"><a class="btn btn-primary signin-a" href="${pageContext.request.contextPath}/user/login"><spring:message code="label.navigation.login.link"/> <span class="glyphicon glyphicon-log-in" aria-hidden="true"/></a></li>
                	<li class="signup_li" ><a class="btn btn-danger signup-a" href="${pageContext.request.contextPath}/user/register"><spring:message code="label.navigation.registration.link"/> <span class="glyphicon glyphicon-registration-mark" aria-hidden="true"/></a></li>
                </sec:authorize>
                <sec:authorize access="isAuthenticated()">
                    <li>
                        <form action="${pageContext.request.contextPath}/logout" method="POST">
                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                            <button type="submit" class="btn btn-primary logout-btn"><spring:message code="label.navigation.logout.link"/>  <span class="glyphicon glyphicon-log-out" aria-hidden="true"/>
                            </button>
                        </form>
                    </li>
                </sec:authorize>
